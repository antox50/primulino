...
module.exports = {
    ...
    callback: async(message, args, client) => {
        ...
        const totalCashInWallet = db.get(`money_${user.id}`) // Same As balance.js // Get User Money In Wallet
        ...
        if(args[0] === 'all') {  // If Someone Does `+dep all` All Money To Bank
            let ciao = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(totalCashInWallet)
        const embed = new MessageEmbed()
        .setAuthor(`${user.user.username} Deposit`, user.user.displayAvatarURL({ dynamic: true }))
        .setTimestamp()
        .setColor('RANDOM')
        .setDescription(`
<@${user.user.id}> Have Deposited **${ciao}** To Bank
        `)
