...
module.exports = {
    commands: ['buymutt', 'buy mutt', 'buy Mutt'], // You Can Keep Any Name
 ...
 // You Can Keep Any Name
            ...
            const amount = 8000 // Amount Of Laptop: $10,000
            ...
            if(bal <amount) { // If Balance In Wallet Is Less Then Amount Of Laptop
                return message.reply(`You don't have enough money(\`$8,000\`) in wallet to buy role`)
            } else {
              
                message.member.roles.add('1013400024501207081');
                ...
                .setColor('RANDOM')
                .setDescription(`
<@${user.id}> Successfully purchased role mutt for \`$8,000\` 👍
                `)
                .setFooter('Shop')
                message.channel.send(embed)
                db.add(`loser${user.id}`, 1) // Add 1 Laptop To User
